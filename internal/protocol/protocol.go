...
// применяет только DoThreshold и DoSkip -- остальные ему не адресованы, и
...
// Оси: о ком высказывание. Оба наших глагола ездят только с request.
const (
...
type Action struct {
	...
	// счёту, множитель 1 + delta/100. Value -- только при DoNote; этот
	// инспектор его не применяет, но разбирает: действие обязано доехать до
	// аудита целиком.
