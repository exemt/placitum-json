/*
 * Проверка вызова по документу OpenAPI.
 *
 * Порядок один и тот же на обеих фазах: найти операцию -> проверить то, что
 * включено профилем -> вернуть исход. Проверки идут по отдельности намеренно:
 * у каждой свой исход и своя политика, а один общий «валиден / не валиден»
 * склеил бы «прислал не тот тип» с «прислал не то значение», и оператор не смог
 * бы разрешить первое, не разрешив второе.
 */
...
// Request проверяет запрос. Возвращаемый исход -- не вердикт: во что его
// превратить, решает политика профиля.
...
/*
 * requestBody возвращает находки и признак «тело не описано операцией». Второе
 * -- отдельный исход, а не нарушение схемы: схемы, по которой судить, здесь
 * просто нет.
 */
func (c *Contract) requestBody(in *Input, op *v3.Operation, item *v3.PathItem,
	...
	if len(in.Body) == 0 {
		// Тела нет, а операция его требует -- это нарушение контракта, и
		// заметить его должны мы: валидатор тела на пустом теле молчит.
...
// Response проверяет ответ приложения. Состояния фазы запроса здесь нет и не
// нужно: операция находится по методу и пути, а они приезжают в сообщении.
func (c *Contract) Response(in *Input, ch config.ResponseChecks, opts Options) Result {
	...
	if !found {
		if !ch.Status {
			// Кода нет в спеке, и проверять его не просили: сверять тело не с
			// чем, и это не находка.
			return res
		}
	...
	if declared == nil || declared.Content == nil {
		// Код описан, тела для него не описано -- сверять не с чем.
		return res
	}
...
func (c *Contract) response(in *Input) *http.Response {
	...
	if resp.Header.Get("Content-Type") == "" {
		// Валидатор выбирает описание тела по типу; без заголовка он не найдёт
		// ни одного и промолчит, а молчание здесь означало бы «сошлось».
...
/*
 * finish сводит находки в исход. Правило простое: проверка, которая нашла
 * нарушение схемы, важнее той, которая не нашла схемы вовсе -- иначе один
 * недописанный в спеке тип тела прятал бы все настоящие расхождения операции.
 */
...
/*
 * withDocument разбирает тело второй раз -- но только когда профиль просит
 * хеши значений. Без этой просьбы разбор не нужен: значение никуда не уходит,
 * а лишний проход по мегабайтному телу на горячем пути не бесплатен.
 */
...
// operationName -- то, что видно в аудите и в логе. Метод плюс шаблон пути:
// конкретный путь запроса лежит в kind=request у модуля, дублировать его сюда
